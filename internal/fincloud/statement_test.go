...
func TestFetchSavingAccountStatementContract(t *testing.T) {
	...
	body.Store(`{"status":"ok","data":{"result":[]}}`)
	empty, err = client.FetchSavingAccountStatement(context.Background(), "S-1")
...
func TestFetchSavingAccountStatementRejectsMalformedContractsWithoutLeakingBody(t *testing.T) {
	...
	invalid := []struct {
		...
		{body: `{"status":"failed","data":{"result":{"mutasi":[]}}}`},
		{body: `{"status":"ok"}`},
		{body: `{"status":"ok","data":null}`},
		{body: `{"status":"ok","data":[]}`},
		{body: `{"status":"ok","data":{}}`},
		{body: `{"status":"ok","data":{"result":null}}`},
		{body: `{"status":"ok","data":{"result":[{}]}}`, stage: "statement_result"},
		{body: `{"status":"ok","data":{"result":[1]}}`, stage: "statement_result"},
		{body: `{"status":"ok","data":{"result":["anything"]}}`, stage: "statement_result"},
		{body: `{"status":"ok","data":{"result":{}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":null}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":{}}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":[null]}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":[{"trx_rate":"1"}]}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":[{"mid_rate_dc":1}]}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":[{"keterangan":1}]}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":[{"tgltransaksi":"31-08-2026"}]}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":[{"jam":"1:02:03"}]}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":[{"saldoawal":"1,23.00"}]}}}`},
		{body: `{"status":"ok","data":{"result":{"mutasi":[]}}} {}`},
	}
	for _, candidate := range invalid {
		...
		if err == nil {
			t.Fatalf("invalid statement accepted: %s", candidate.body)
		}
		...
		if candidate.stage != "" && diagnostic.DecodeStage != candidate.stage {
			t.Fatalf("decode stage=%q want=%q error=%v", diagnostic.DecodeStage, candidate.stage, err)
		}
