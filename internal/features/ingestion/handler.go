...
type coordinator interface {
	SubmitRunAll(context.Context, core.CalendarDate, core.CalendarDate, int, ingestionrun.Trigger, string, *uint64) (uint64, error)
...
func (handler *Handler) SubmitRunAll(writer http.ResponseWriter, request *http.Request) {
	...
	id, err := handler.coordinator.SubmitRunAll(request.Context(), from, to, 3, ingestionrun.TriggerDirect, "web:"+middleware.GetReqID(request.Context()), &actor)
	if err != nil {
