...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  module_default.data("csvUpload", () => ({
    ...
    select(files, dropped = false) {
      ...
      if (file.size > 15e7) return this.reject("CSV file must be 150 MB or smaller.", !dropped);
      if (dropped) {
    ...
    reject(message, clear2) {
      if (clear2) this.remove();
      this.error = message;
    },
    remove() {
  ...
  module_default.data("reportResult", (elementID) => ({
    result: { columns: [], rows: [] },
    page: 1,
    pageSize: 50,
    init() {
      ...
      (_a = this.result).columns || (_a.columns = []);
      (_b = this.result).rows || (_b.rows = []);
    },
    get pages() {
      return Math.max(1, Math.ceil(this.result.rows.length / this.pageSize));
    },
    get pageRows() {
      const start2 = (this.page - 1) * this.pageSize;
      return this.result.rows.slice(start2, start2 + this.pageSize);
    },
    get pageLabel() {
  ...
  module_default.data("reportParameters", (elementID, reportID, enabled) => ({
    definitions: [],
    states: {},
    controllers: {},
    async init() {
    ...
    async load(key, initial = false) {
      ...
      try {
        ...
        if (error2.name === "AbortError" || this.stateFor(key).generation !== generation) return;
    ...
    dynamicChanged(key, event) {
      ...
      const selected = definition.type === "multiple_option" ? [...this.$root.querySelectorAll(`[name="param_${key}"]:checked`)].map((input) => input.value) : event.target.value ? [event.target.value] : [];
      this.setState(key, { selected, present: true });
      this.cascade(key);
    },
    async cascade(changedKey) {
      const changed = /* @__PURE__ */ new Set([changedKey]);
